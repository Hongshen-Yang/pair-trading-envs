'''
This Env inherent from env_restrict_thres
But allows the RL to decide the investment amount
'''
...
class RL_Restrict_TradeEnv(gym.Env):
    ...
    def _take_action(self):
        ...
        sys=TradingSystem(self.df, self.holdings, self.trade_step, cash=self.cash, amt=self.action*self.cash)
        # Convert free-amount-env action to fixed-amount-env action
        if self.action<0:
            action=0
        elif self.action>0:
            action=2
        ...
        if self.position==0 and action==0:
            ...
            self.cash, self.holdings = sys.open_position(action)
            ...
            # Close position
            self.cash, self.holdings = sys.close_position()
        elif self.position==2 and action==2:
            # Do nothing
            pass
